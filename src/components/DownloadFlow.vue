...
<template>
  <!--
    Rendered where it sits in the markup, inside `.download-wrapper`.

    An earlier version teleported it to the body and measured the card's box
    back onto a fixed-position backdrop, so the dim would follow the section as
    the page scrolled. That is more machinery than the effect is worth, and it
    had to give up and dim the whole page once the section scrolled mostly out
    of view. Absolute positioning inside the wrapper gets the same scoped dim
    with no measuring, no scroll listener, and no whole-page fallback: the
    dialog belongs to the card and scrolls away with it.
  -->
  <div
    class="flow-backdrop"
    role="dialog"
...
<style scoped>
.flow-backdrop {
  /* The wrapper is `position: relative`, so this covers exactly it. */
  position: absolute;
  inset: 0;
